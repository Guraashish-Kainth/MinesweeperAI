...
class Minesweeper:
    ...
    def setup(self):
        ...
        pool = [*product(range(len(self.tiles)), range(len(self.tiles[0])))]
        random_indices = random.sample(pool, mine_count)
        for pairs in random_indices:
            self.tiles[pairs[0]][pairs[1]]['isMine'] = True            
    ...
    def onClick(self, tile):        
        ...
        for row in self.tiles.keys():
            for column in self.tiles[row].keys():                
                if self.tiles[row][column]['state'] == 1:
                    boxes += 1

        if tile["isMine"] == True:
            # end game
            self.gameOver(False)
            return -20, True, boxes, time
        ...
        if self.clickedCount == (SIZE_X * SIZE_Y) - self.mines:
            self.gameOver(True)            
            self.boxCount = boxes
            return 50, self.gameOver, boxes, time        
        
        self.boxCount = boxes
        return 10, False, boxes, time
